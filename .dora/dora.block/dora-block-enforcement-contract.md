...
for file in $FILES; do
    ...
    if [[ $file == *.py || $file == *.yaml || $file == *.yml || $file == *.md || $file == *.json ]]; then
        ...
        if ! grep -q "DORA PROTOCOL METADATA" "$file"; then
            echo "❌ $file: Missing DORA block"
            FAILED=1
        else
            echo "✓ $file: DORA block present"
        fi
    fi
done
...
if [ $FAILED -eq 1 ]; then
    echo ""
    echo "❌ DORA validation failed. Please add DORA blocks to all files."
    echo ""
    echo "Run: python scripts/auto_add_dora_blocks.py"
    exit 1
fi

echo "✅ All DORA blocks validated"
exit 0
```
