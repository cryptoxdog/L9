    ...
    try:
        ...
        packet_in = PacketEnvelopeIn(
            ...
            payload={
                ...
                "result": payload.result[:2000] if payload.result else None,  # Truncate large results
    ...
    if task.source == "slack" and task.channel:
        try:
            ...
            if payload.logs:
                ...
                recent_logs = payload.logs[-5:] if len(payload.logs) > 5 else payload.logs
            ...
            logger.info(f"[MAC-AGENT] Posted result for task {task_id} to Slack channel {task.channel}")
        except Exception as e:
