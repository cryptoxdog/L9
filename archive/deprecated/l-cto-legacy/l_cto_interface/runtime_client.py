...
class RuntimeClient:
    ...
    def execute_directive(self, directive: dict) -> dict:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/directive", json=directive, timeout=30
            )
    ...
    def list_modules(self) -> dict:
        ...
        """
        try:
            response = self.session.get(f"{self.base_url}/modules", timeout=5)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def get_introspection(self) -> dict:
        ...
        """
        try:
            response = self.session.get(f"{self.base_url}/introspection", timeout=5)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
