    ...
    def init(self):
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                if schema_count > 0:
                    logger.info(
                        f"Neo4j schemas detected: {schema_count} constraints found"
                    )
                else:
                    logger.warning(
                        "No Neo4j schemas detected - run kg_bootstrap.py to deploy"
                    )
    ...
    def upsert(self, node_type: str, data: dict) -> dict:
        ...
        try:
            with self.driver.session() as session:
                ...
                return {"success": True, "node": dict(node["n"]) if node else None}
        except Exception as e:
