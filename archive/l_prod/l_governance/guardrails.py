...
def _detect_drift(directive: dict, output: dict):
    ...
    success_rate = sum(1 for e in recent_entries if e.get("success")) / len(recent_entries)
    
    # Check for sudden drops in success rate
    if success_rate < 0.6:
        logger.warning(f"Behavioral drift detected: success rate dropped to {success_rate:.1%}")
    
    # Check for repeated failures on same command
    current_command = directive.get("command")
    recent_failures = [e for e in recent_entries if e.get("command") == current_command and not e.get("success")]
