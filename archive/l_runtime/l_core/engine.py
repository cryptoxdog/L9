...
try:
    ...
    logging.warning(f"Memory layer not available - running without persistent memory: {e}")
...
class LEngine:
        ...
        try:
            ...
            if not gov_check["passed"]:
                ...
                if gov_check["action"] == "escalate":
                    logger.error(f"ESCALATING TO IGOR: {gov_check.get('escalation_reason')}")
                    return {
                        "success": False,
    ...
    def _recursion_governance_check(self, evaluation: dict) -> dict:
        ...
        ceo_keywords = ["override igor", "final decision", "ceo authority", "strategic vision"]
        role_aligned = not any(keyword in conclusion.lower() for keyword in ceo_keywords)
        ...
        architecture_aligned = not any(violation in conclusion.lower() for violation in architecture_violations)
        checks["architecture_alignment"] = architecture_aligned
        
        if not architecture_aligned:
            logger.warning("Architecture misalignment: Attempting to violate boundaries")
        ...
        if self.last_directive_signature:
            # Compare patterns
            drift_score = self._calculate_drift(self.last_directive_signature, current_signature)
        ...
        if len(self.execution_history) >= 3:
            ...
            consistency_score = self._check_historical_consistency(evaluation, recent_outputs)
            checks["historical_consistency"] = consistency_score > 0.7  # Threshold
            
            if not checks["historical_consistency"]:
                logger.warning(f"Historical inconsistency detected: score={consistency_score:.2f}")
        else:
        ...
        safety_passed = not any(keyword in conclusion.lower() for keyword in dangerous_keywords)
        ...
        if not all_passed:
            ...
            self_correctable = all(f in ["drift_detection", "historical_consistency"] for f in failures)
    ...
    def _get_directive_signature(self, evaluation: dict) -> dict:
        ...
        reasoning = evaluation.get("reasoning", {})
        return {
            "conclusion_keywords": set(reasoning.get("conclusion", "").lower().split()[:10]),
    ...
    def _check_historical_consistency(self, current_eval: dict, recent_outputs: list) -> float:
        """Check consistency with recent executions."""
        if not recent_outputs:
            return 1.0
        
        current_conclusion = current_eval.get("reasoning", {}).get("conclusion", "").lower()
        
        # Check for contradictions with recent conclusions
        contradictions = 0
        for output in recent_outputs:
            past_conclusion = output.get("output", {}).get("evaluation", {}).get("reasoning", {}).get("conclusion", "").lower()
            
            # Simple contradiction detection
            if "not" in current_conclusion and past_conclusion.replace(" not ", " ") in current_conclusion:
                contradictions += 1
