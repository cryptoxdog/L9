...
def extract_print_content(line: str, lines: List[str], line_idx: int) -> Tuple[Optional[str], int]:
...
def convert_print_to_logger(line: str, lines: List[str], line_idx: int, indent: str) -> Tuple[str, int]:
...
def lint_file(file_path: Path, fix: bool = False) -> LintResult:
    ...
    for line_num, line in enumerate(lines):
        ...
        if import_insert_pos == -1 and (stripped.startswith('import ') or stripped.startswith('from ')):
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if (line_num - 1) in skip_lines:
            new_lines.append("")  # Replace with blank line (content was merged into previous)
            continue
        ...
        for forbidden, message in FORBIDDEN_IMPORTS.items():
            ...
            if re.search(pattern, line):
                ...
                if fix and forbidden == "logging":
                    ...
                    new_line = re.sub(
                        rf'^import\s+{forbidden}\b',
                        'import structlog',
                        new_line
                    )
                    new_line = re.sub(
                        rf'^from\s+{forbidden}\b',
                        'from structlog',
                        new_line
                    )
        ...
        if re.search(r'logger\s*=\s*logging\.getLogger', line) and not line.strip().startswith('#'):
            result.add_error(line_num, "logging.getLogger", "Use 'structlog.get_logger(__name__)' instead")
        ...
        is_comment_only = line.strip().startswith('#') or (not line_before_comment.strip())
        if "test_" not in str(file_path) and not is_comment_only and not in_docstring_pass2:
                ...
                if re.search(pattern, line_before_comment):
                    ...
                    if '"' in line_before_comment or "'" in line_before_comment:
                        ...
                        for i, char in enumerate(line_before_comment):
                            if char in ('"', "'") and (i == 0 or line_before_comment[i-1] != '\\'):
                                if quote_char is None:
                                    ...
                                    in_string = False
                                    quote_char = None
                            elif not in_string and line_before_comment[i:i+5] == "print":
                                found_print = True
                                break
                    ...
                    if fix:
                        converted, lines_consumed = convert_print_to_logger(line, lines, line_num - 1, indent_str)
                        if converted != line:
                            new_line = converted
                            modified = True
                            result.fixed = True
    ...
    if fix and modified and not has_logger_definition:
        ...
        if logger_insert_pos == -1:
            ...
            if last_import_pos >= 0:
                ...
                if logger_insert_pos < len(new_lines) and new_lines[logger_insert_pos].strip():
                    logger_line = "\nlogger = structlog.get_logger(__name__)\n"
                else:
...
def main() -> int:
    ...
    if total_errors == 0:
        ...
        if args.fix:
            ...
            logger.error(f"❌ Found {total_errors} issue(s) in {len([r for r in all_results if r.has_errors])} file(s)")
