...
class PacketEnvelopeIn(BaseModel):
    ...
    """
    packet_type: str = Field(..., min_length=1, description="Semantic category (e.g., event, insight)")
    payload: dict[str, Any] = Field(..., description="Flexible JSON-like structure")
    metadata: Optional[dict[str, Any]] = Field(None, description="Optional metadata")
    provenance: Optional[dict[str, Any]] = Field(None, description="Optional provenance (parent_packet, source_agent)")
    confidence: Optional[dict[str, Any]] = Field(None, description="Optional confidence (score, rationale)")
...
class MemoryClient:
    ...
    async def write_packet(
        ...
        logger.info(f"Packet written: id={result.packet_id}, tables={result.written_tables}")
