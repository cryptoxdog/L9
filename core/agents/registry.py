...
class AgentRegistry:
    ...
    def _parse_agent_config(self, data: dict[str, Any], source: Path) -> Optional[AgentConfig]:
        """
            ...
            if "tools" in data:
                for tool_data in data["tools"]:
                    if isinstance(tool_data, str):
                        ...
                        if not tool_id:
                            logger.warning("Tool binding missing tool_id, skipping: %s", tool_data)
                            continue
                        tools.append(ToolBinding(
                            tool_id=tool_id,
                            display_name=tool_data.get("display_name") or tool_data.get("name"),
                            description=tool_data.get("description"),
                            input_schema=tool_data.get("input_schema") or tool_data.get("parameters", {}),
                        ))
            ...
            "agents": [
                {
                    ...
                    "display_name": config.metadata.get("display_name", config.agent_id),
                    "model": config.model,
