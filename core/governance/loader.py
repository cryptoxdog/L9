    ...
    def get_policies_for_action(self, action: str) -> list[Policy]:
        ...
        for policy in self.policies:
            if not policy.actions:
                ...
                self._action_matches(action, pattern) for pattern in policy.actions
            ):
                matching.append(policy)
        return matching
...
__dora_block__ = {
    ...
    "audit_trail": True,
    "purpose": "Provides loader components including PolicyLoadError, InvalidPolicyError, PolicyLoader",
    "dependencies": [],
}
