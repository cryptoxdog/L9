...
KERNEL_HASH_FILE = Path("l9_private/kernel_hashes.json")
...
def compute_kernel_hashes(base_path: str = "l9_private") -> Dict[str, str]:
    """
...
class IntegrityChange:
    ...
    def __init__(self, path: str, change_type: str, old_hash: Optional[str] = None, new_hash: Optional[str] = None):
        self.path = path
...
def get_detailed_changes(base_path: str = "l9_private") -> List[IntegrityChange]:
    """
    ...
    for path, current_hash in current.items():
        if path not in existing:
            changes.append(IntegrityChange(
                path=path,
...
def verify_specific_file(file_path: str) -> Tuple[bool, Optional[str]]:
    ...
    existing = load_kernel_hashes()
    relative_path = str(path.relative_to(Path.cwd())) if path.is_absolute() else file_path
...
def initialize_kernel_hashes(base_path: str = "l9_private", force: bool = False) -> int:
    """
