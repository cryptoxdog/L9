...
class ResearchJobSpec(BaseModel):
    ...
    regions: list[str] = Field(..., min_length=1, description="Geographic regions to search")
    max_results: int = Field(default=50, ge=1, le=1000, description="Maximum results to retrieve")
...
class Query(BaseModel):
    ...
    priority: int = Field(default=1, ge=1, le=10, description="Query priority (1=highest)")
...
class QueryPlan(BaseModel):
    ...
    queries: list[Query] = Field(..., min_length=1, description="Ordered list of queries")
    strategy: str = Field(..., description="Search strategy (e.g., 'breadth_first', 'depth_first')")
    constraints: dict[str, Any] = Field(default_factory=dict, description="Plan constraints")
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class RetrievalBatch(BaseModel):
    ...
    latency_ms: float = Field(..., ge=0, description="Retrieval latency in milliseconds")
...
class IntegrationResult(BaseModel):
    ...
    parsed_objects: list[ParsedObject] = Field(..., description="Extracted and validated objects")
    integration_summary: dict[str, Any] = Field(..., description="Summary of integration actions")
    ...
    def success_rate(self) -> float:
        ...
        if total == 0:
            return 0.0
        valid = sum(1 for p in self.parsed_objects if p.validation_status == ValidationStatus.VALID)
        return valid / total
