...
async def pass_1_plan_queries(state: ResearchState) -> ResearchState:
    ...
    logger.info(f"Pass 1: Planning queries for job {state.job_spec.job_id if state.job_spec else 'N/A'}")
...
"""
        ...
        for query in state.query_plan.queries:
            variables = {
                ...
                "max_results": state.query_plan.constraints.get("max_results_per_query", 10)
            }
            
            rendered = template.format(**variables)
            
            superprompts.append(Superprompt(
...
async def pass_3_execute_retrieval(
    ...
    try:
        ...
        for prompt in state.superprompts:
            ...
            retrieval_batches.append(RetrievalBatch(
...
async def pass_4_extract_results(
    state: ResearchState,
    extraction_backend: Optional[ExtractionBackend] = None
) -> ResearchState:
    """
    ...
    try:
        ...
        for batch in state.retrieval_batches:
            for raw in batch.raw_responses:
                ...
                if errors:
                    status = ValidationStatus.INVALID if len(errors) > 1 else ValidationStatus.PARTIAL
...
async def pass_5_integrate_results(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        valid_count = sum(1 for p in state.parsed_objects if p.validation_status == ValidationStatus.VALID)
        integration_summary = {
            "total_processed": len(state.parsed_objects),
            "valid_objects": valid_count,
            "success_rate": valid_count / len(state.parsed_objects) if state.parsed_objects else 0.0,
        ...
        logger.info(f"Pass 5 complete: Integration finished with {valid_count} valid objects")
        return state
