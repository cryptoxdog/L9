...
if docker ps | grep -q "traefik"; then
    TRAEFIK_ROUTING=$($COMPOSE_CMD logs traefik 2>/dev/null | grep -i "l9\|quantumaipartners" | tail -3)
    if [ -n "$TRAEFIK_ROUTING" ]; then
        echo -e "${GREEN}✅ Traefik logs show routing activity${NC}"
        echo "$TRAEFIK_ROUTING"
        ((PASSED++))
    else
        echo -e "${YELLOW}⚠️  No routing activity in Traefik logs${NC}"
        echo "   This may be normal if no requests have been made"
        ((FAILED++))
    fi
else
...
if docker network ls | grep -q "l9net"; then
    CONTAINERS_ON_NETWORK=$(docker network inspect l9net --format '{{range .Containers}}{{.Name}} {{end}}' 2>/dev/null | wc -w)
    if [ "$CONTAINERS_ON_NETWORK" -ge 2 ]; then
        echo -e "${GREEN}✅ Both containers on l9net network ($CONTAINERS_ON_NETWORK containers)${NC}"
        docker network inspect l9net --format '{{range .Containers}}{{.Name}} {{end}}' 2>/dev/null
        ((PASSED++))
    else
