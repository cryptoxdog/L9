# ============================================================================
# L9 MODULE SPEC GENERATOR PROMPT FOR MANUS — v1.1.0
# ============================================================================
...
# Output: Complete Module-Spec-v2.1.yaml for EVERY module in L9 roadmap
# ============================================================================
# CHANGELOG v1.1.0:
#   - Updated template to v2.1.0 (from v2.0.0)
#   - Added Section 11: MANDATORY STANDARDS (tool_id, structlog, httpx)
#   - Replaced all "optional" language with explicit INCLUDE_WHEN/OMIT_WHEN
#   - Added prescriptive patterns (R7-R9) to GENERATION_RULES
#   - Added quality gates for v2.1 compliance (gate_5, gate_6, gate_7)
#   - Updated notes_for_perplexity with 4 new v2.1 items
# ============================================================================
...
MISSION:
  objective: |
    ...
    Each spec MUST follow Module-Spec-v2.1.yaml template EXACTLY.
    Each spec MUST be ready for Perplexity to generate wire-ready code.
    
  deliverables:
    - "One Module-Spec-v2.1.yaml per module"
    ...
    - "All specs include MANDATORY STANDARDS section"
  ...
  success_criteria:
    ...
    - "All specs enforce tool_id, structlog, httpx"
...
# ============================================================================
# SECTION 4: SPEC TEMPLATE (USE THIS EXACTLY — v2.1.0)
# ============================================================================
SPEC_TEMPLATE: |
  # ============================================================================
  ...
  module:
    # CANONICAL IDENTIFIER — Use this everywhere (logs, packets, metrics, configs)
    id: "{{module_id}}"
    # DISPLAY NAME ONLY — Never use for programmatic references
    name: "{{Module Name}}"
  ...
  repo:
    ...
    # FILE MANIFEST — Explicit inclusion criteria for each file
    allowed_new_files:
      # ALWAYS INCLUDE — Core module files
      - path: "api/{{module}}_adapter.py"
        include: "ALWAYS"
        
      # CONDITIONAL — External API client
      - path: "api/{{module}}_client.py"
        include_when: "Module makes outbound API calls to external services (Slack, WhatsApp, Twilio, etc.)"
        omit_when: "Module only receives webhooks OR only calls internal AIOS endpoints"
        
      # ALWAYS INCLUDE — API routes
      - path: "api/routes/{{module}}.py"
        include: "ALWAYS"
        
      # CONDITIONAL — Custom packet storage
      - path: "memory/{{module}}_ingest.py"
        include_when: "Module has custom packet transformation OR custom thread_uuid generation"
        omit_when: "Module uses standard substrate_service.write_packet without modification"
        
      # ALWAYS INCLUDE — Adapter tests
      - path: "tests/test_{{module}}_adapter.py"
        include: "ALWAYS"
        
      # CONDITIONAL — Ingest tests (mirrors ingest file)
      - path: "tests/test_{{module}}_ingest.py"
        include_when: "memory/{{module}}_ingest.py is included"
        omit_when: "memory/{{module}}_ingest.py is omitted"
  ...
  environment:
    ...
    # CONDITIONAL — Include only if module has external API calls
    conditional:
      - name: "{{OPTIONAL_VAR}}"
        description: "{{What it's for}}"
  ...
  # ============================================================================
  # SECTION 11: MANDATORY STANDARDS (NON-NEGOTIABLE)
  # ============================================================================
  standards:
    ...
    identity:
      ...
      value: "{{module_id}}"  # Same as module.id
      usage: |
        Use module.id as tool_id in ALL of these:
        - Log events: log.info("event", tool_id="{{module_id}}")
        - Packet metadata: packet.tool_id = "{{module_id}}"
        - Metrics labels: counter.labels(tool_id="{{module_id}}")
        - Config references: config["{{module_id}}"]
      forbidden: "NEVER use module.name for programmatic references"
    
    # LOGGING — structlog ONLY
    logging:
      library: "structlog"
      forbidden: ["logging", "print", "logger"]
      setup_pattern: |
        import structlog
        log = structlog.get_logger(__name__)
        
        # CORRECT usage
        log.info("request_received", tool_id="{{module_id}}", event_id=event_id)
        log.error("handler_error", tool_id="{{module_id}}", error=str(e))
      anti_pattern: |
    ...
    # HTTP CLIENT — httpx ONLY
    http_client:
      library: "httpx"
      forbidden: ["aiohttp", "requests", "urllib"]
      async_pattern: |
        import httpx
  ...
  notes_for_perplexity:
    ...
    # NEW v2.1 NOTES — MANDATORY
    - "Use tool_id (module.id) as canonical identifier in logs, packets, metrics"
    - "Use structlog for all logging — NEVER stdlib logging"
    - "Use httpx for all HTTP calls — NEVER aiohttp"
    - "Follow file manifest include_when/omit_when criteria exactly"
...
GENERATION_RULES:
  ...
  # NEW v1.1 RULES — PRESCRIPTIVE PATTERNS
  R7_USE_TOOL_ID:
    rule: "ALWAYS use module.id as tool_id in logs, packets, metrics, configs"
    correct: |
      log.info("request_received", tool_id="slack.adapter", event_id=event_id)
      packet.metadata["tool_id"] = "slack.adapter"
      counter.labels(tool_id="slack.adapter").inc()
    wrong: |
      log.info("request_received", tool_name="Slack Adapter")
      packet.metadata["module_name"] = "Slack Integration"
      counter.labels(name="slack").inc()

  R8_USE_STRUCTLOG:
    rule: "ALL logging MUST use structlog — NEVER stdlib logging or print"
    correct: |
      import structlog
      log = structlog.get_logger(__name__)
      
      log.info("event_name", tool_id="module.id", field="value")
      log.error("handler_error", tool_id="module.id", error=str(e), traceback=tb)
    wrong: |
      import logging
      logger = logging.getLogger(__name__)
  ...
  R9_USE_HTTPX:
    rule: "ALL HTTP calls MUST use httpx — NEVER aiohttp or requests"
    correct_async: |
      import httpx
  ...
  R10_FILE_MANIFEST_COMPLIANCE:
    rule: "File manifest MUST use explicit include_when/omit_when — NEVER 'optional'"
    correct: |
      - path: "api/{{module}}_client.py"
        include_when: "Module makes outbound API calls to external services"
        omit_when: "Module only receives webhooks"
    wrong: |
      - "api/{{module}}_client.py"  # Optional
      - "api/{{module}}_client.py"  # Include if needed
...
OUTPUT_FORMAT:
  structure: |
    ...
    ```yaml
    [Complete Module-Spec-v2.1.yaml with ALL fields filled]
    ```
  ...
  completeness_check:
    before_outputting_each_spec:
      ...
      - "[ ] standards section is complete (tool_id, structlog, httpx)"
      - "[ ] File manifest uses include_when/omit_when (no 'optional')"
      - "[ ] notes_for_perplexity has all 11 items"
...
QUALITY_GATES:
  ...
  # NEW v1.1 QUALITY GATES
  gate_5_standards_compliance:
    check: "Spec includes complete standards section"
    action: "Verify standards.identity, standards.logging, standards.http_client all present"
    required_fields:
      - "standards.identity.canonical_identifier = 'tool_id'"
      - "standards.identity.value = module.id"
      - "standards.logging.library = 'structlog'"
      - "standards.http_client.library = 'httpx'"
    
  gate_6_file_manifest_determinism:
    check: "All conditional files have explicit include_when/omit_when"
    action: "Verify no files use 'optional', 'if needed', or other ambiguous language"
    forbidden_words: ["optional", "if needed", "when appropriate", "as needed", "may include"]
    
  gate_7_notes_completeness:
    check: "notes_for_perplexity includes all 11 required items"
    action: "Count notes_for_perplexity items — must be >= 11"
    required_items:
      - "REPO_CONTEXT_PACK imports"
      - "PacketEnvelopeIn"
      - "UUIDv5 for thread_id"
      - "injected services"
      - "request.app.state"
...
FINAL_OUTPUT:
  format: |
    ...
    - All specs compliant with v2.1 standards
  ...
  attestation: |
    ...
    | File manifest deterministic | ✅ |
    | Notes complete (11 items) | ✅ |
...
INSTRUCTIONS:
  role: |
    ...
    5. Enforce v2.1 standards in every spec
  ...
  workflow: |
    ...
    3. For EACH module in MODULE_ROADMAP:
       ...
       - Verify against GENERATION_RULES (R1-R10)
       - Pass QUALITY_GATES (gate_1 through gate_7)
  ...
  critical_rules: |
    ...
    - Do NOT use word "optional" — use include_when/omit_when
    - Do NOT omit standards section
    - Every spec MUST be Perplexity-ready
    - Every spec MUST pass all 7 quality gates

# ============================================================================
# END OF MANUS PROMPT v1.1.0
# ============================================================================
