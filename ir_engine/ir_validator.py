...
class IRValidator:
    ...
    def _validate_referential_integrity(
        ...
        for intent_id, intent in graph.intents.items():
            if intent.parent_intent_id and intent.parent_intent_id not in all_intent_ids:
                result.add_error(
        ...
        for constraint_id, constraint in graph.constraints.items():
            for applies_to_id in constraint.applies_to:
                if applies_to_id not in all_intent_ids and applies_to_id not in all_action_ids:
                    result.add_warning(
        ...
        for action_id, action in graph.actions.items():
            if action.derived_from_intent and action.derived_from_intent not in all_intent_ids:
                result.add_warning(
    ...
    def _validate_constraints(self, graph: IRGraph, result: IRValidationResult) -> None:
        ...
        for constraint_id, constraint in graph.constraints.items():
            if constraint.constraint_type.value == "false" and constraint.status == ConstraintStatus.ACTIVE:
                result.add_warning(
    ...
    def _validate_dependencies(self, graph: IRGraph, result: IRValidationResult) -> None:
    ...
    def _validate_completeness(self, graph: IRGraph, result: IRValidationResult) -> None:
