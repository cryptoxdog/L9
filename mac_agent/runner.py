...
def execute_command(command: str) -> tuple[str, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            output = f"Exit code: {result.returncode}\n{error_msg}" if error_msg else f"Exit code: {result.returncode}"
...
async def poll_and_execute():
    ...
    logger.info("Mac Agent V2 runner starting. Polling file-based task queue every 3 seconds")
    ...
    while True:
        ...
        try:
            ...
            if task_type == "email_task":
                ...
                executor = AutomationExecutor()
                result = await executor.run_steps(steps, headless=False, task_id=task_id)
            ...
            if result.get("status") == "error" and not result.get("screenshots"):
                if PYTHON_AUTOGUI_AVAILABLE:
                    try:
                        ...
                        desktop_screenshot = screenshot_dir / f"desktop_error_{int(time.time())}.png"
            ...
            try:
                ...
                channel = metadata.get("channel", user)  # Fallback to user if no channel
                
                if user or channel:
                    post_result(channel or user, task, result)
                    logger.info(f"Posted result for task {task_id} to Slack")
                else:
                    logger.warning(f"No user/channel in task {task_id} metadata, skipping Slack post")
            except Exception as e:
                logger.error(f"Failed to post result to Slack for task {task_id}: {e}", exc_info=True)
            ...
            logger.error(f"Unexpected error processing task {task_id if task_id else 'unknown'}: {e}", exc_info=True)
            ...
            if task_id and task:
                try:
                    failure_result = {
                        "status": "error",
                        "logs": [{"step": 0, "action": "runner_error", "status": "error", "details": str(e), "timestamp": datetime.utcnow().isoformat()}],
                        "screenshots": [],
                    ...
                    if PYTHON_AUTOGUI_AVAILABLE:
                        try:
                            screenshot_dir = Path(os.path.expanduser(f"~/.l9/mac_tasks/screenshots/{task_id}"))
                            screenshot_dir.mkdir(parents=True, exist_ok=True)
                            desktop_screenshot = screenshot_dir / f"desktop_crash_{int(time.time())}.png"
                            pyautogui.screenshot(str(desktop_screenshot))
                    ...
                    # Save failure JSON
                    try:
                        completed_file = Path(os.path.expanduser(f"~/.l9/mac_tasks/completed/{task_id}.json"))
                        with open(completed_file, "w") as f:
                            json.dump({"task": task, "result": failure_result}, f, indent=2)
                    except Exception:
                        pass
