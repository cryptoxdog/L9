...
class CodeExtractor(BaseExtractor):
    ...
    PATTERNS = [
        ...
        # # api/mcp/mcp_auth.py
        (r'^#\s+([\w/\-\.]+\.(?:py|js|ts|yaml|yml|json|jsx|tsx))\s*$', 'comment_header'),
        ...
        (r'^[🔥📁📄✨💡]\s+([\w/\-\.]+\.(?:py|js|ts|yaml|yml|json|jsx|tsx))', 'emoji_marker'),
        
        # Pattern 4: Triple backticks with filename
        # ```python:path/to/file.py
        (r'```(?:python|javascript|typescript|yaml|json)?:([\w/\-\.]+\.(?:py|js|ts|yaml|yml|json|jsx|tsx))', 'code_block_with_path'),
    ]
        ...
        while i < len(lines):
            ...
            for pattern, pattern_type in self.PATTERNS:
                ...
                if match:
                    ...
                    code_start = i + 1
                    code_content = self.extract_code_content(lines, code_start, pattern_type)
    ...
    def extract_code_content(self, lines: List[str], start_idx: int, pattern_type: str) -> str:
        ...
        while i < len(lines):
            ...
            if not in_code_block and (
                ...
                any(re.match(p[0], line.strip()) for p in self.PATTERNS)
            ):
