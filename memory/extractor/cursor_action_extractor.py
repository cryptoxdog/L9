...
class CursorActionExtractor(BaseExtractor):
    ...
    def extract_yaml_configs(self, content: str) -> Dict[str, Dict]:
        ...
        yaml_match = re.search(
            r"_g_version:\s*([\d.]+).*?name:\s*L9-L-Clone-Core", content, re.DOTALL
        )
        if yaml_match:
            # Try to extract structured YAML
            configs["L9-L-Clone-Core"] = self.parse_inline_yaml(
                content[yaml_match.start() : yaml_match.end() + 500]
            )
...
__dora_block__ = {
    ...
    "audit_trail": True,
    "purpose": "Implements CursorActionExtractor for cursor action extractor functionality",
    "dependencies": [],
}
