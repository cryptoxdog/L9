        ...
        for token in decision_tokens:
            if ":" in token:
                ...
                confidence = confidence_scores.get(
                    key, confidence_scores.get("routing", 0.7)
                )
                ...
                if confidence >= 0.7:
                    insights.append(
                        ExtractedInsight(
                            ...
                            source_packet=packet_id,
                            trigger_world_model=key
                            in ("action", "route_to", "update_model"),
                        )
                    )
...
__dora_block__ = {
    ...
    "audit_trail": True,
    "purpose": "Implements InsightExtractionPipeline for insight extraction functionality",
    "dependencies": [],
}
