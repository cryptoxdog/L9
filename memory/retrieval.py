...
class RetrievalPipeline:
    ...
    async def hybrid_search(
        ...
        for hit in semantic_hits:
            ...
            matching_packet = next(
                (p for p in filtered_packets if str(p.packet_id) == packet_id),
                None
            )
            combined.append({
                "score": hit.score,
                ...
                "packet_id": packet_id,
                "payload": hit.payload,
                "packet": matching_packet.model_dump(mode="json") if matching_packet else None,
            })
    ...
    async def fetch_facts(
        ...
        if source_packet:
            ...
            facts = await self._repository.get_facts_by_subject(subject, predicate, limit)
        else:
            # Fetch recent facts
            async with self._repository.acquire() as conn:
                rows = await conn.fetch(
    ...
    async def replay_chain(
        ...
        for item in lineage["chain"]:
            ...
            if include_reasoning:
                traces = await self._repository.get_reasoning_traces(packet_id=packet_id, limit=1)
                if traces:
