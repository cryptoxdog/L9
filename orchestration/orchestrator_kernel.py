...
class OrchestratorKernel:
    ...
    async def run_pipeline(
        ...
        try:
            ...
            normalized_task, normalized_context = await self._phase_ingest(task, context)
            ...
            try:
                ...
                packet = PacketEnvelopeIn(
                    ...
                    payload={
                        ...
                        "graph_id": str(result.graph.graph_id) if result.graph else None,
            ...
            while chain.current_step < len(chain.steps):
                ...
                if step.status == "failed":
                    if self._config.retry_on_failure:
                        retries = 0
                        while retries < self._config.max_retries and step.status == "failed":
                            retries += 1
                            logger.info(f"Retrying step {step.name} (attempt {retries})")
                            step.status = "pending"
                            step.error = None
                            await self._execute_chain_step(chain, step)
    ...
    def _create_checkpoint(self, chain: ExecutionChain) -> None:
        ...
        logger.debug(f"Checkpoint created for chain {chain.chain_id} at step {chain.current_step}")
