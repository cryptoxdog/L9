...
class ApplyEngine:
    ...
    async def apply(self, upgrade: Upgrade, allow_downtime: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, change in enumerate(upgrade.changes):
                logger.info(f"Applying change {i+1}/{len(upgrade.changes)}: {change.get('type')}")
                ...
                if not success:
                    ...
                    return {"success": False, "error": f"Failed to apply change {i+1}"}
            
            self._applied_upgrades[upgrade.id] = backup
            logger.info(f"Successfully applied upgrade: {upgrade.id}")
            return {"success": True, "message": f"Upgrade {upgrade.id} applied successfully"}
    ...
    async def rollback(self, upgrade_id: str) -> Dict[str, Any]:
        ...
        if upgrade_id not in self._applied_upgrades:
            return {"success": False, "error": f"No backup found for upgrade {upgrade_id}"}
        ...
        try:
            ...
            return {"success": True, "message": f"Upgrade {upgrade_id} rolled back successfully"}
    ...
    async def _create_backup(self, upgrade: Upgrade) -> Dict[str, Any]:
        ...
        backup = {"upgrade_id": upgrade.id, "version_from": upgrade.version_from, "changes": []}
    ...
    async def _apply_db_migration(self, change: Dict[str, Any], allow_downtime: bool) -> bool:
    ...
    async def _apply_service_restart(self, change: Dict[str, Any], allow_downtime: bool) -> bool:
        """Restart a service."""
        try:
