...
class EvolutionOrchestrator(IEvolutionOrchestrator):
    ...
    async def apply_upgrades(
        ...
        for upgrade in sorted_upgrades:
            execution = await self._apply_single_upgrade(upgrade, request.allow_downtime)
    ...
    async def rollback_upgrade(self, upgrade_id: str) -> Dict[str, Any]:
        ...
        if not execution:
            return {"success": False, "message": f"Upgrade {upgrade_id} not found in history"}
        
        if execution.status != UpgradeStatus.DEPLOYED:
            return {"success": False, "message": f"Upgrade {upgrade_id} is not deployed (status: {execution.status})"}
    ...
    async def _apply_single_upgrade(self, upgrade: Upgrade, allow_downtime: bool) -> UpgradeExecution:
