        ...
        for hit in result.hits if result and hasattr(result, "hits") else []:
            ...
            if isinstance(payload, dict):
                ...
                if isinstance(envelope, dict):
                    ...
                    if segment in tags or segment in envelope_tags or f"memory_{segment}" in packet_type or f"memory_{segment}" in envelope_type:
...
async def memory_write(
    ...
    try:
        ...
        if result and result.packet_id:
            logger.info(f"Wrote to memory segment {segment}: packet_id={result.packet_id}")
