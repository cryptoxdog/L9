...
class BatchSpecGenerator:
    ...
    async def process_batch(self, modules: List[tuple]) -> List[dict]:
        ...
        tasks = [
            self.client.generate_spec(name, desc, semaphore)
    ...
    async def run(self, modules: List[tuple], dry_run: bool = False) -> dict:
        ...
        batch_count = (len(modules) + self.config.batch_size - 1) // self.config.batch_size
...
async def main():
    ...
    parser.add_argument("--description", type=str, help="Module description (with --module)")
