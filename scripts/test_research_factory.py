...
async def test_research_endpoint():
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        try:
            ...
            print(f"   ✅ Research Factory enabled: {features.get('quantum_research', False)}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   ⚠️  Research service not initialized (503)")
                print(f"   → Check: MEMORY_DSN environment variable set?")
                print(f"   → Check: Database running?")
            else:
