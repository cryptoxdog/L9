...
class PerplexityRequest:
    ...
    search_domain_filter: list[str] = field(default_factory=list)  # e.g., ["docs.python.org"]
    ...
    def validate(self) -> list[str]:
        ...
        if len(self.query.split()) < 5:
            issues.append("Query too short - add 2-3 words of context for better results")
        ...
        if any(p in self.query.lower() for p in bad_patterns):
            issues.append("Use search_domain_filter parameter instead of prompt instructions")
...
class PerplexityClient:
    ...
    async def search(self, request: PerplexityRequest) -> PerplexityResponse:
        ...
        if issues:
            log.warning("perplexity_request_issues", issues=issues, query=request.query[:50])
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                log.error("perplexity_error", status=response.status_code, error=error_text)
    ...
    def _build_payload(self, request: PerplexityRequest) -> dict[str, Any]:
        ...
        if request.search_context_size != SearchContextSize.MEDIUM:
            web_search_options["search_context_size"] = request.search_context_size.value
    ...
    def _parse_response(self, data: dict, model: PerplexityModel) -> PerplexityResponse:
        ...
        total_cost = cost_info.get("total_cost", 0.0) if isinstance(cost_info, dict) else 0.0
    ...
    async def quick_search(self, query: str) -> PerplexityResponse:
        ...
        """
        return await self.search(PerplexityRequest(
            query=query,
