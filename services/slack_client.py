...
def post_result(user: str, task: dict, result: dict):
    ...
    try:
        ...
        task_id = task.get("task_id", task.get("metadata", {}).get("task_id", "unknown"))
        ...
        successes = sum(1 for log in logs if isinstance(log, dict) and log.get("status") == "success")
        failures = sum(1 for log in logs if isinstance(log, dict) and log.get("status") == "error")
        ...
        if logs:
            ...
            for log in logs[:3]:
                if isinstance(log, dict):
                    ...
                    log_details.append(f"• {action}: {log_status}" + (f" - {details[:50]}" if details else ""))
                else:
        ...
        if task_type == "email_task":
            ...
            for step in steps:
                ...
                if action in ["list_messages", "get_message", "draft_email", "send_email", "reply_to_email"]:
                    operation = action
                    break
            ...
            if "messages" in result_data:
                ...
                for msg in messages[:3]:  # Show first 3
                    message_parts.append(f"  • {msg.get('subject', 'No subject')} from {msg.get('from', 'Unknown')}")
                if len(messages) > 3:
            ...
            if "message" in result_data:
                ...
                if msg.get('attachments'):
                    message_parts.append(f"*Attachments:* {len(msg['attachments'])} file(s)")
            ...
            if "message_id" in result_data:
                message_parts.append("")
                message_parts.append(f"✅ Email sent successfully")
        ...
        for artifact in artifacts[:2]:  # Show first 2 artifacts
            ...
            if artifact.get("pdf"):
                ...
                if summary:
                    ...
                    message_parts.append("")
                    message_parts.append(f"PDF Fields ({artifact_name}): {fields_preview}...")
            ...
            if artifact.get("transcription"):
                ...
                if trans_text:
                    ...
                    message_parts.append("")
                    message_parts.append(f"Transcription ({artifact_name}): {preview}...")
        ...
        for screenshot_path in screenshots:
            if screenshot_path and os.path.exists(screenshot_path):
                try:
                    ...
                    logger.info(f"[SLACK] Uploaded screenshot: {os.path.basename(screenshot_path)}")
                except Exception as e:
                    logger.error(f"[SLACK] Failed to upload screenshot {screenshot_path}: {e}")
        
        # If no screenshots were uploaded but we have screenshot_path (backward compat)
        if not uploaded_files and result.get("screenshot_path") and os.path.exists(result.get("screenshot_path")):
            try:
        ...
        logger.info(f"[SLACK] Posted result for task {task_id} to {user} ({len(uploaded_files)} screenshots)")
        return response
