...
class TestWorldModelRepositoryIntegration:
    ...
    def test_repository_has_crud_methods(self):
        ...
        with patch('world_model.repository.get_pool'):
            ...
            assert hasattr(repo, 'get_entity') or hasattr(repo, 'create_node') or hasattr(repo, 'add_node')
            assert hasattr(repo, 'list_entities') or hasattr(repo, 'get_node') or hasattr(repo, 'find_node')
