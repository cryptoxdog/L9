...
class SeedLoader:
    ...
    async def write_packet_to_substrate(
        ...
        try:
            packet_in = PacketEnvelopeIn(
                ...
                provenance=packet.provenance.model_dump() if packet.provenance else None,
            )
    ...
    async def run(
        ...
        if reflection_path.exists():
            ...
            logger.debug(f"Reflection memory file not found (optional): {reflection_path}")
        ...
        if cross_task_path.exists():
            ...
            logger.debug(f"Cross-task graph file not found (optional): {cross_task_path}")
        ...
        # Calculate totals
        total_entities = sum(
            r.get("ingest_result", {}).get("entities_added", 0) or 0
            for r in results
        )
        total_relations = sum(
            r.get("ingest_result", {}).get("relations_added", 0) or 0
            for r in results
        )
...
async def run_seed_loader(
    ...
    Returns:
        Loading summary
    """
    from memory.substrate_repository import SubstrateRepository, get_substrate_repository
